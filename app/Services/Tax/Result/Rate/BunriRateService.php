...
final class BunriRateService
{
    ...
    public function minRateForSeparatedTaxes(array $payload, string $period): ?float
    {
        ...
        $shortTermRaw = PayloadAccessor::intOrNull($payload, $shortTermKey);
        $shortTermAmount = $shortTermRaw !== null
            ? PayloadAccessor::floorToThousands(PayloadAccessor::nonNegativeFloat($shortTermRaw))
            : null;

        if ($shortTermAmount !== null && $shortTermAmount > 0.0) {
            return self::SHORT_TERM_RATE;
        }

        $shortTermIsZero = $shortTermAmount === null || $shortTermAmount === 0.0;
        ...
        foreach (self::OTHER_KEYS as $pattern) {
            ...
            $valueRaw = PayloadAccessor::intOrNull($payload, $key);

            if ($valueRaw === null) {
                continue;
            }

            $amount = PayloadAccessor::floorToThousands(PayloadAccessor::nonNegativeFloat($valueRaw));
